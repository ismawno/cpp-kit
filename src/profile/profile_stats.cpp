...
long long profile_stats::duration_per_call() const
{
    return m_duration_per_call;
}
...
{
    return m_relative_calls;
}
std::uint32_t profile_stats::total_calls() const
{
    return m_total_calls;
}

float profile_stats::relative_percent() const
{
    return m_relative_percent;
}
float profile_stats::total_percent() const
{
    return m_total_percent;
}

const std::unordered_map<const char *, profile_stats> &profile_stats::children() const
{
    return m_children;
}
} // namespace perf